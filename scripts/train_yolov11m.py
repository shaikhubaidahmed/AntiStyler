...
def main():
    print("=== YOLOv11-M CLEAN BASELINE TRAINING ===")
    ...
    pretrained_ckpt = os.path.join(project_root, "yolo11m.pt")
    
    expected_hash = "d5ffc1a674953a08e11a8d21e022781b1b23a19b730afc309290bd9fb5305b95"
    actual_hash = compute_sha256(pretrained_ckpt)
    ...
    print(f"Pretrained checkpoint size: {os.path.getsize(pretrained_ckpt) / (1024*1024):.2f} MB")
    ...
    results = model.train(
        ...
        batch=16,
        device=0,
        project=os.path.join(project_root, "experiments", "yolov11m_gtsdb"),
        name="run",
        exist_ok=True,
        save=True,
        save_json=True
    ...
    t1 = time.time()
    train_duration = (t1 - t0) / 3600
    print(f"Training completed in {train_duration:.2f} hours")
    ...
    # Move the predictions.json to root of experiments/yolov11m_gtsdb for safekeeping
    ...
    try:
        ...
        for p in preds:
            ...
            if img_name in img_name_to_id:
                ...
                # If not found, just try to extract digits
