...
DATA_YAML = os.path.join(PROJECT_ROOT, "configs", "datasets", "gtsdb_yolov7_defended.yaml")

from evaluate_coco_gtsdb import evaluate_coco

def main():
    print("Running YOLOv7 evaluation on defended images (test.py)...")
    cmd = [
        "python", "test.py",
        "--weights", WEIGHTS,
        "--data", DATA_YAML,
    ...
    yolov7_preds_json = os.path.join(EXPERIMENT_DIR, "eval_defended", "best_predictions.json")
    ...
    fixed_preds_path = os.path.join(EXPERIMENT_DIR, "eval_defended", "best_predictions_fixed.json")
    with open(fixed_preds_path, 'w') as f:
        json.dump(fixed_preds, f)
    
    print("\nRunning COCO Evaluation on Defended Predictions...")
    stats = evaluate_coco(gt_json, fixed_preds_path)
    
    print(f"\nDefended COCO mAP@0.50: {stats[1]}")
