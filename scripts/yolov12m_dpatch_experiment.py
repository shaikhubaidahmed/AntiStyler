...
def main():
    print("=== YOLOv12-M DPATCH ATTACK EXPERIMENT ===")
    ...
    for i, img_name in enumerate(test_images):
        ...
        for epoch in range(EPOCHS):
            ...
            for out in outputs:
                B, C, H, W = out.shape
                sy, sx = get_spatial_grid_cells(H, W)
    ...
    for p in preds:
        ...
        # Fixing 1-indexed to 0-indexed categories
    ...
    # Extract clean metrics from previous report
    clean_map50 = 0.9217
...
INTERPRETATION:
The DPatch attack significantly degraded YOLOv12-M's detection capability. The COCO mAP50 dropped from {clean_map50:.4f} to {attack_map50:.4f} (an absolute degradation of {deg_abs:.4f} and a relative degradation of {deg_rel:.2f}%), indicating high vulnerability to spatially localized adversarial patches. The prediction count increased from {clean_det} to {num_detections}.
"""
